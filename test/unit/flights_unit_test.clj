...
(deftest validate-flights-filter
  (testing "Filtering flights from JFK to LAX"
    (let [result (logic.flight/find-flights {:airport-departure :JFK :airport-destination :LAX} flight-data-fixture)]
  ...
  (testing "Filtering unexistent flight from GRU to IPA, it must return an empty array"
    (let [result (logic.flight/find-flights {:airport-departure :GRU :airport-destination :IPA} flight-data-fixture)]
      (is (= 0 (count result)) "Shouldn't return any flight"))))
